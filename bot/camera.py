...
class Camera(abc.ABC):
    ...
    def __init__(self, cam_config: CameraConfig, config: ConfigWrapper, klippy: Klippy, logging_handler: logging.Handler) -> None:
        ...
        self.fourcc: str = cam_config.fourcc
...
class NumpyCamera(Camera):
    ...
    def take_video(self) -> tuple[BytesIO, BytesIO, int, int]:
        with self._camera_lock:
            ...
            self._release_capture()
            _encode_frames(frame_list, filepath, self.fourcc, self._video_duration, self._transform_frame)
            os_nice(0)
...
class MjpegCamera(Camera):
    ...
    def take_video(self) -> tuple[BytesIO, BytesIO, int, int]:
        ...
        with self._camera_lock:
            ...
            _encode_frames(frame_list, filepath, self.fourcc, self._video_duration, self._image_to_frame)
            os_nice(0)
